...
local shaders = {
    chameleo = exports["exv_shaders"]:createShader("vehPaint/chameleo.fx", 1, 30),
    matte    = exports["exv_shaders"]:createShader("vehPaint/matte.fx", 1, 30),
    gloss    = exports["exv_shaders"]:createShader("vehPaint/gloss.fx", 2, 30, false, "vehicle"),
    chrome   = exports["exv_shaders"]:createShader("vehPaint/car_paint.fx", 1, 50, false)
}

local textures = {
	cube_env256   = dxCreateTexture("assets/fx/cube_env256.dds"),
	tempBody 	  = dxCreateTexture("assets/fx/tempBody.png"),
	textureCube   = dxCreateTexture("assets/fx/reflx.dds"),
	textureFringe = dxCreateTexture("assets/fx/color.png"),
...
local function getVehicleColorByName(vehicle, name)
    ...
    elseif name == "hlbody" then
    	local RGB = getElementData(vehicle, "exv_carPaint.colorTypeRGB") 
...
function setupColorPreview() -- СОХРАНЯЕМ ЦВЕТ КРАСКИ ПРИ ВХОДЕ 
    ...
    initialColors = {
    	...
    	colorType  = getElementData(vehicle, 'exv_carPaint.colorType') or false
    }
end
...
function setVehiclePreviewColor(r, g, b)
    ...
    if guiCheckBoxGetSelected(UI.chekBoxColor2) then -- цвет блеска
    	vehicle:setData('exv_carPaint.colorTypeRGB', {r / 255, g / 255, b / 255})
    end
...
function resetVehiclePreview()
    ...
    if initialColors.hlbody then
    	local r3, g3, b3 = unpack(initialColors.hlbody)
   		-- цвет блеска --
   		vehicle:setData('exv_carPaint.colorTypeRGB', {r3, g3, b3})
    end

    if not initialColors.colorType then
    	return vehicle:setData('exv_carPaint.colorType', false)
    end

    vehicle:setData('exv_carPaint.colorType', initialColors.colorType)
...
function setVehiclePreviewFX(colorType) -- ПРЕДПРОСМОТР ПОКРАСКИ
    ...
    vehicle:setData('exv_carPaint.colorType', colorType)
end
...
local function setVehicleColorFX(vehicle)
	...
	local colorType = getElementData(vehicle, 'exv_carPaint.colorType')
...
addEventHandler("onClientElementDataChange", root, function(data)
	...
	if data == 'exv_carPaint.colorTypeRGB' then
		if shadersCars[source] then
			local RGB = getElementData(source, "exv_carPaint.colorTypeRGB") 
        	local r, g, b = unpack(RGB)
    		dxSetShaderValue(shadersCars[source], "paintColor2", {r, g, b})
    	end
	end

	if data == 'exv_carPaint.colorType' then
		removeColors(source)
