addEvent("exv_skinshop.onPlayerSkinChange",true)
addEventHandler("exv_skinshop.onPlayerSkinChange",getRootElement(),function(skinid, skinprice)
...
local function onPickupHitEnter(player)
	local pickup_data = getElementData(source, 'exv_skinshop.PickupInfo')
	...
	setTimer(function()
		...
		triggerClientEvent(player, 'exv_skinShop.music', player, not pickup_data.exit)
...
addEventHandler('onResourceStart', getResourceRootElement(), function()
	for _, shop in ipairs(skinShopTable) do
		...
		local pickup_entrance = createPickup(
			...
			shop.entrance.z, 
		3, 1318, 1)
		setElementData(pickup_entrance, 'exv_skinshop.PickupInfo', 
			{ 
				x = shop.exit.x, 
				y = shop.exit.y,
				z = shop.exit.z, 
		...
		setElementData(pickup_exit, 'exv_skinshop.PickupInfo', 
			{ 
				x = shop.entrance.x, 
				y = shop.entrance.y,
				z = shop.entrance.z, 
