...
addEventHandler("onClientElementStreamIn", root, function()
	if getElementType(source) == "pickup" then
		local data = getElementData(source, "exv_skinshop.PickupInfo")
		if data then
...
addEventHandler("onClientElementStreamOut", root, function()
	if getElementType(source) == "pickup" and getElementData(source, "exv_skinshop.PickupInfo") then
...
addEventHandler("onClientGUIClick", root, function()
	...
	if source == button_out or source == button_buy then
		...
		if source == button_buy then
			...
			local price = skinShopTable[index]["ID"][typeSkin][numberSkin][2]
			triggerServerEvent("exv_skinshop.onPlayerSkinChange", localPlayer, id, price)
		end
...
-- --
-- https://digisky.ru/upload/medialibrary/instrumental.mp3 --
local soundBackGround 

addEvent("exv_skinShop.music", true)
addEventHandler("exv_skinShop.music", root, function(state)
	if state then
		soundBackGround = playSound('https://digisky.ru/upload/medialibrary/instrumental.mp3', true)
...
addEventHandler('onClientResourceStart', resourceRoot, function()
	for i, M in ipairs(skinShopTable) do
		...
		createBlip(M.entrance.x, M.entrance.y, M.entrance.z, 45, 0, 255, 0, 0, 255, 0, 450)
	end
end)