...
local function getWorldPositionFromMap(mapX, mapY) -- позиция в мире по позиции на карте
...
local function drawMap() -- КАРТА
	...
	dxSetRenderTarget(mapRenderTarget)
		...
		dxDrawImage(pX-(20/ZOOM)/2, pY-(20/ZOOM)/2, 20/ZOOM, 20/ZOOM, "assets/blips/2.png", (-rotzP)%360, 0, 0, tocolor(255, 255, 255, 255)) -- позиция игрока
	dxSetRenderTarget()
...
local function showMap()
	if not getElementData(localPlayer, "exv_player.ACTIVE_UI") == 'map' then return end
	...
	if not MAP_VISIBLE then 
		getPlayerFromMap()
		triggerEvent("exv_showui.blurShaderStart", localPlayer)
		addEventHandler("onClientRender", root, drawMap)
		setElementData(localPlayer, "exv_player.ACTIVE_UI", "map")
	else
		CLICK_MOVE = false
		triggerEvent("exv_showui.blurShaderStop", localPlayer)
	...
	triggerEvent("exv_showui.setVisiblePlayerUI", getRootElement(), not MAP_VISIBLE)
	triggerEvent("exv_showui.setVisiblePlayerComponentUI", getRootElement(), "map", true)
end
